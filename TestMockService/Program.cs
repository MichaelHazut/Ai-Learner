            ...
            try
            {
                ...
                for (int i = 0; i < 50; i++)
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        {
                            string fixAttemt = JsonService.FixMissingBracket(cleanedJson);
                            stItem = JsonConvert.DeserializeObject<StudyMaterial>(fixAttemt);
                            await Console.Out.WriteLineAsync("broken Json Added");
                            await File.AppendAllTextAsync("../../../logs/material_Log.txt", fixAttemt + ",\n");
                        }
